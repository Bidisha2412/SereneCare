...
# ML Pipeline
from src.ml.inference       import CareWatchPredictor
from src.ml.video_pipeline  import RealTimeVideoPipeline
...
health_predictor = CareWatchPredictor(
    model_path   = os.path.join(os.path.dirname(__file__), "output", "carewatch_model.lgbm"),
    scaler_path  = os.path.join(os.path.dirname(__file__), "output", "carewatch_scaler.pkl"),
    cooldown_seconds = 30,
)

_rt_video = RealTimeVideoPipeline(window=30, stride=15)
...
def _monitoring_loop():
    ...
    while _system_status["monitoring"]:
        ...
        # Corrected Indentation for ML Pipeline
        feat_df = _rt_video.update(frame)

        if feat_df is not None and not feat_df.empty:
            feat_row = feat_df.iloc[0].to_dict()
            result   = health_predictor.predict_from_features(feat_row)
...
def get_risk():
    return jsonify(_last_risk_prediction or {
        "label": 0, "class_name": "Normal", "confidence": 0, "severity": "none", "probabilities": {}
    })
